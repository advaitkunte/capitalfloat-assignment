...
CREATE INDEX country_region ON country (region);
...
CREATE INDEX countrylanguage_name ON countrylanguage (language);
...
/*CITY*/
-- Fix out of sync SERIAL
-- https://stackoverflow.com/a/244265
...
LOCK TABLE city IN EXCLUSIVE MODE;
-- Update the sequence
SELECT setval('city_id_seq', COALESCE((SELECT MAX(id)+1 FROM city), 1), false);
COMMIT;
...
BEGIN;
-- protect against concurrent inserts while you update the counter
LOCK TABLE country IN EXCLUSIVE MODE;
-- Update the sequence
SELECT setval('country_id_seq', COALESCE((SELECT MAX(id)+1 FROM country), 1), false);
COMMIT;
...
-- What is the result?
SELECT MAX(id) FROM countrylanguage;

-- Then run...
-- This should be higher than the last result.
SELECT nextval('countrylanguage_id_seq');
...
BEGIN;
-- protect against concurrent inserts while you update the counter
LOCK TABLE countrylanguage IN EXCLUSIVE MODE;
-- Update the sequence
SELECT setval('countrylanguage_id_seq', COALESCE((SELECT MAX(id)+1 FROM countrylanguage), 1), false);
COMMIT;